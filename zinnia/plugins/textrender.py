...
def render_formula(formula, folder, font_size=11, latex_class='article'):
    ...
    if os.path.exists(os.path.join(folder, "%s.png" % hash)):
        return "%s.png" % hash
    ...
    status = os.system("convert -density 120 -trim -transparent \"#FFFFFF\" "
        "formula.ps formula.png")
    assert 0 == status, tempdir

    os.chdir(curpath)

    shutil.copyfile(os.path.join(tempdir, "formula.png"),
        os.path.join(folder, "%s.png" % hash))
